		           ...
		           float    *SphereRadius)
{
	...
	for (i = 0; i < N; i++)
	{
		for (c = XAXIS; c <= ZAXIS; c++)
		{
			di2[c] = SQR(d[i * 3 + c]);                 // di2[c] = SQR(d[i][c]);

			 s[c] += d[i * 3 + c];                      // s[c]  += d[i][c];
			s2[c] += di2[c];
			s3[c] += di2[c] * d[i * 3 + c];             // s3[c] += di2[c] * d[i][c];

			Population[d[i * 3 + c] > 0.0f][c]++;       // Population[d[i][c] > 0.0f][c]++;
		}

		xy_sum += d[i * 3 + XAXIS] * d[i * 3 + YAXIS];  // xy_sum += d[i][XAXIS] * d[i][YAXIS];
		xz_sum += d[i * 3 + XAXIS] * d[i * 3 + ZAXIS];  // xz_sum += d[i][XAXIS] * d[i][ZAXIS];
		yz_sum += d[i * 3 + YAXIS] * d[i * 3 + ZAXIS];  // yz_sum += d[i][YAXIS] * d[i][ZAXIS];

		x2sum[YAXIS] += di2[XAXIS] * d[i * 3 + YAXIS];  // x2sum[YAXIS] += di2[XAXIS] * d[i][YAXIS];
		x2sum[ZAXIS] += di2[XAXIS] * d[i * 3 + ZAXIS];  // x2sum[ZAXIS] += di2[XAXIS] * d[i][ZAXIS];

		y2sum[XAXIS] += di2[YAXIS] * d[i * 3 + XAXIS];  // y2sum[XAXIS] += di2[YAXIS] * d[i][XAXIS];
		y2sum[ZAXIS] += di2[YAXIS] * d[i * 3 + ZAXIS];  // y2sum[ZAXIS] += di2[YAXIS] * d[i][ZAXIS];

		z2sum[XAXIS] += di2[ZAXIS] * d[i * 3 + XAXIS];  // z2sum[XAXIS] += di2[ZAXIS] * d[i][XAXIS];
		z2sum[YAXIS] += di2[ZAXIS] * d[i * 3 + YAXIS];  // z2sum[YAXIS] += di2[ZAXIS] * d[i][YAXIS];
	}
