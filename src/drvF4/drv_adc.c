...
void adcInit(void)
{
  ...
  //GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL ;
    ...
    ADC_RegularChannelConfig(ADC2, ADC2_CHANNEL, 5,  ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC3_CHANNEL, 6,  ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC4_CHANNEL, 7,  ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC7_CHANNEL, 8,  ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC2_CHANNEL, 9,  ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC3_CHANNEL, 10, ADC_SampleTime_480Cycles);
    ADC_RegularChannelConfig(ADC2, ADC4_CHANNEL, 11, ADC_SampleTime_480Cycles);
...
float mxr9150XAxis(void)
{
	...
	for (i = ADC_PIN_2_CONVERTED_VALUE; i < ADC_PIN_2_CONVERTED_VALUE + 13; i += 4)
	    adcSum += (int16_t)adc2ConvertedValues[i];
...
float mxr9150YAxis(void)
{
	...
	for (i = ADC_PIN_3_CONVERTED_VALUE; i < ADC_PIN_3_CONVERTED_VALUE + 13; i += 4)
	    adcSum += (int16_t)adc2ConvertedValues[i];
...
float voltageMonitor(void)
{
	...
	for (i = ADC_PIN_7_CONVERTED_VALUE; i < ADC_PIN_7_CONVERTED_VALUE + 13; i += 4)
	    adcSum += (int16_t)adc2ConvertedValues[i];
