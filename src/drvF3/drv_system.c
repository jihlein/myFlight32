...
semaphore_t systemOperational = false;
...

    {
        ...
        if (((frameCounter + 1) % COUNT_10HZ) == 0)
        {
			if (eepromConfig.externalHMC5883 == 3)
			    newMagData = readMag5883();
			else
			    newMagData = readMag5983();
		}
...
void systemInit(void)
{
	...
	gpsPortClearBuffer       = &uart1ClearBuffer;
    gpsPortNumCharsAvailable = &uart1NumCharsAvailable;
    gpsPortPrintBinary       = &uart1PrintBinary;
    gpsPortRead              = &uart1Read;

    openLogPortPrintF        = &uart2PrintF;
    ...
    cliPortPrintF("\nMy Flight 32 (Naze32Pro) Firmware V%s, Build Date " __DATE__ " "__TIME__" \n", __MYFLIGHT32_VERSION);
    ...
    tasks100Hz_initialize();
    tasks500Hz_initialize();
}
