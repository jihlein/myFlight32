...
void mixerCLI()
{
    ...
    while(true)
    {
		...
		switch(mixerQuery)
		{
        	    ...
        	    if (eepromConfig.mixerConfiguration == MIXERTYPE_FREE)
                {
        	        ...
        	        for ( index = 0; index < eepromConfig.freeMixMotors; index++ )
        	        {
        	    	    cliPortPrintF("Motor%1d  %6.3f  %6.3f  %6.3f  %6.3f\n", index + 1,
               	    		 	                                                eepromConfig.freeMix[index     ],
               	    		 	                                                eepromConfig.freeMix[index +  8],
                	    		 	                                            eepromConfig.freeMix[index + 16],
                	    		 	                                            eepromConfig.freeMix[index + 24]);
        	        }
            ...
            case 'K': // Read Free Mix Matrix Element
                if (eepromConfig.mixerConfiguration == MIXERTYPE_FREE)
                {
                    ...
                    if ((rows > 0) && (rows < 9) && (columns > 0) && (columns < 5))
                    	eepromConfig.freeMix[(rows - 1) + 8 * (columns - 1)] = readFloatCLI();
                    else
                    {
						tempFloat = readFloatCLI();
			...
			case '?':
			   	cliPortPrint("\n");
			   	cliPortPrint("'a' Mixer Configuration Data               'A' Set Mixer Configuration              A0 thru 4, see ff32_AQ32.h\n");
